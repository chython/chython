...
def test_tautomer_fix():
    ...
    # After tautomer fixing, one N should have 0 H and the other should have 1 H
    n_with_h = 0
    n_without_h = 0
    for n, atom in mol.atoms():
        if atom.atomic_number == 7:  # nitrogen
            assert len(mol._bonds[n]) == 2  # should have 2 bonds
            if mol._hydrogens[n] == 1:
                n_with_h += 1
            elif mol._hydrogens[n] == 0:
                n_without_h += 1
    
    assert n_with_h == 1  # one nitrogen should have 1 hydrogen
    assert n_without_h == 1  # one nitrogen should have 0 hydrogens
...
def test_quinone_exclusion():
    ...
    initial_double_bonds = [(n, m) for n, m, bond in mol.bonds() if bond.order == 2]
    assert not mol.thiele()  # should return False
    
    # Check that double bonds remain as double bonds
    final_double_bonds = [(n, m) for n, m, bond in mol.bonds() if bond.order == 2]
    assert len(final_double_bonds) == len(initial_double_bonds)
...
def test_charged_systems():
    ...
    for n, atom in mol.atoms():
        if atom.atomic_number == 7:  # nitrogen
            assert mol._charges[n] == 1  # should have +1 charge
...
def test_freak_rules():
    ...
    mol = smiles('N1C=CN2C=CC=CC1=2')  # special N-fused system in Kekule form
    assert mol.thiele()
...
def test_tetracyclic_systems():
    ...
    # Only the 6-membered ring should be aromatic
    aromatic_bonds = 0
    rings_by_size = {len(ring): ring for ring in mol.sssr}
    
    for n, m, bond in mol.bonds():
        if bond.order == 4:  # aromatic bond
            aromatic_bonds += 1
            # Check that this bond belongs to the 6-membered ring
            if 6 in rings_by_size:
                six_ring = rings_by_size[6]
                assert n in six_ring and m in six_ring
    
    assert aromatic_bonds == 6  # six aromatic bonds in benzene ring
    assert result  # should return True since part of the molecule is aromatic
...
def test_ferrocene_like():
    ...
    mol = smiles('[C-]1C=CC=C1')  # cyclopentadienyl anion
    assert mol.thiele()
    
    # Check charge preservation and aromaticity
    charged_carbons = 0
    for n, atom in mol.atoms():
        if atom.atomic_number == 6 and mol._charges[n] == -1:
            charged_carbons += 1
    assert charged_carbons == 1
...
def test_complex_heterocycles():
    ...
    result = mol.thiele()
    assert result  # use stored result instead of calling thiele() again
    
    # Check bonds in each ring separately
    benzene_ring = {1, 2, 3, 4, 5, 6}  # benzene part
    imidazole_ring = {3, 4, 7, 8, 9}  # imidazole part
    thiophene_ring = {10, 11, 12, 13, 14}  # thiophene part
    
    for n, m, bond in mol.bonds():
        # Check if both atoms of the bond are in the same ring
        if (n in benzene_ring and m in benzene_ring) or \
           (n in imidazole_ring and m in imidazole_ring) or \
           (n in thiophene_ring and m in thiophene_ring):
            assert bond.order == 4, f"Bond {n}-{m} should be aromatic"


def test_complex_bridged_systems():
    # Test bridged aromatic systems
    
    # Basic bridged systems that should work
    # Naphthalene
    mol = smiles('C1=CC=C2C=CC=CC2=C1')  # in Kekule form
    assert mol.thiele()
    for n, m, bond in mol.bonds():
        assert bond.order == 4
    
    # Quinoline
    mol = smiles('C1=CC=C2C=CC=NC2=C1')  # in Kekule form
    assert mol.thiele()
    for n, m, bond in mol.bonds():
        assert bond.order == 4
    ...
    # Check bonds in each ring separately
    benzene_ring = {1, 2, 3, 4, 5, 6}  # benzene part
    pyridine_ring = {8, 9, 10, 11, 12, 13}  # pyridine part
    
    for n, m, bond in mol.bonds():
        # Check if both atoms of the bond are in the same ring
        if (n in benzene_ring and m in benzene_ring) or \
           (n in pyridine_ring and m in pyridine_ring):
            assert bond.order == 4, f"Bond {n}-{m} should be aromatic"
        # Bonds between rings or to S can remain non-aromatic 