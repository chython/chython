...
def unpack(const unsigned char[::1] data not None):
    ...
    cdef unsigned short *mapping, *cis_trans_1, *cis_trans_2, *connections
    ...
    for i in range(atoms_count):
        ...
        hydrogens = a >> 5
        if hydrogens == 7:
            py_atom._hydrogens = None
        else:
            py_atom._hydrogens = hydrogens
