...
# Format V4 specification::
# Based on V2 with changes in stereochemistry encoding. Compatible with V2 if no stereo information encoded.
# Half of the 3rd atom block's byte changed to support extended stereo.
#
# Big endian bytes order
# 8 bit - 0x04 (current format specification)
# 12 bit - number of atoms
# 12 bit - extended bond stereo block size
#
# Atom block 9 bytes (repeated):
# 12 bit - atom number
# 4 bit - number of neighbors
# 4 bit stereo block. details below.
...
# 010 - triple
# 011 - aromatic
# 111 - special bond (e.g. coordinate)
#
# Atom stereo block:
# ANDx and ORx encode only sign. X value stored in the same order in Stereo group block.
# 0000 [same as V2] - no stereo or unknown
# 0001 - not used
# 0010 - absolute sign False
# 0011 - absolute sign True
# 0100 - sign False OR1 group
# 0101 - sign True OR1 group
# 0110 - sign False AND1 group
# 0111 - sign True AND1 group
# 1000 - sign False OR2 group
# 1001 - sign True OR2 group
# 1010 - sign False AND2 group
# 1011 - sign True AND2 group
# 1100 - sign False ORx group
# 1101 - sign True ORx group
# 1110 - sign False ANDx group
# 1111 - sign True ANDx group
#
# Stereo group block:
# 16 bit group number. hypothetically groups count could be equal to sum of bonds and atoms counts
# first stored groups for atoms in the same order as defined in atom stereo.
# later stored bonds stereo groups.
# allowed values = [3, 2^16).
#
