...
def _make_pids(graph):
    dist = {}
    pid0 = {}
    pid1 = {}
    for n, bs in graph.items():
        dist[n] = dn = {m: 999_999_999 for m in graph}
        pid0[n] = pn = {}
        pid1[n] = {}
        for m in bs:
            dn[m] = 1
    ...
    for k in graph:
        ...
        for i in graph:
            if i == k: continue
            if (dki := dk[i]) == 999_999_999: continue
            di = dist[i]
            p0i = pid0[i]
            p1i = pid1[i]
            p0ik = p0i[k][0][:-1]  # pick only one path and drop atom K

            for j in graph:
                if j == k or j == i: continue
                if (dkj := dk[j]) == 999_999_999: continue
                if (dij := di[j]) > (d := dki + dkj):
                    p = p0ik + pk[j][0]
                    if dij == d + 1:
                        pij = p0i[j]
                        if not any(x[1] == p[1] or x[-2] == p[-2] for x in pij):
                            p1i[j] = pij  # move into pid1
                    else:
                        p1i[j] = []  # init pid1
                    di[j] = d
                    p0i[j] = [p]  # override old paths
                elif dij == d:
                    pij = p0i[j]
                    p = p0ik + pk[j][0]
                    if any(x[1] == p[1] or x[-2] == p[-2] for x in pij): continue
                    pij.append(p)
                elif dij == d - 1:
                    p = p0ik + pk[j][0]
                    if any(x[1] == p[1] or x[-2] == p[-2] for x in p0i[j]): continue
                    pij = p1i[j]
                    if any(x[1] == p[1] or x[-2] == p[-2] for x in pij): continue
                    pij.append(p)
    return pid0, pid1
