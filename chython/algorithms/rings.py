...
class Rings:
    ...
    @cached_property
    def not_special_connectivity(self) -> dict[int, set[int]]:
        """
    ...
    @cached_property
    def skin_graph(self) -> dict[int, set[int]]:
        """
    ...
    @cached_property
    def rings_graph(self) -> dict[int, set[int]]:
        """
...
def _connected_components(bonds: dict[int, set[int] | dict[int, Any]]) -> list[set[int]]:
    atoms = set(bonds)
    components = []
    while atoms:
        start = atoms.pop()
...
def _skin_graph(bonds: dict[int, set[int] | dict[int, Any]]) -> dict[int, set[int]]:
    """
