...
def _rules():
    ...
    rules.append(_make_reactor('aldehyde_to_alcohol', 'aldehyde', '[A:2]-[A:1]'))

    # ketone → secondary_alcohol (NaBH4, LiAlH4)
    rules.append(_make_reactor('ketone_to_alcohol', 'ketone', '[A:2]-[A:1]'))

    # carboxylic_acid → primary_alcohol (LiAlH4)
    rules.append(_make_reactor('acid_to_alcohol', 'carboxylic_acid', '[A:2]-[A:1]'))

    # ester → alcohol (LiAlH4). OR' group is lost.
    rules.append(_make_reactor('ester_to_alcohol', 'ester', '[A:2]-[A:1]'))

    # primary_amide → amine (LiAlH4, BH3)
    rules.append(_make_reactor('amide_to_amine', 'primary_amide', '[A:1]-[A:2]'))

    # secondary_amide → amine (LiAlH4, BH3)
    rules.append(_make_reactor('amide_to_amine', 'secondary_amide', '[A:1]-[A:2]'))

    # nitrile → primary_amine (LiAlH4, H2/cat)
    rules.append(_make_reactor('nitrile_to_amine', 'nitrile', '[A:2]-[A:1]'))

    # nitro → primary_amine (H2/Pd, SnCl2, Fe/HCl)
    rules.append(_make_reactor('nitro_to_amine', 'nitro', '[A:1]'))
